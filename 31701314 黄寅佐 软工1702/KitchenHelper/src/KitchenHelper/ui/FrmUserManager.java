...
public class FrmUserManager extends JDialog implements ActionListener {
	...
	private void reloadUserTable() {
		try {
			List<UserInfo> users = (new UserManager()).loadAllUsers();
			tblData = new Object[users.size()][3];
			for (int i = 0; i < users.size(); i++) {
				tblData[i][0] = users.get(i).getUserNo();
		...
		if (e.getSource() == this.btnAdd) {
			...
			if (JOptionPane.showConfirmDialog(this, "确定重置密码吗？", "确认",
				...
				try {
					...
					JOptionPane.showMessageDialog(null, "密码重置完成, 新密码为账号", "提示", JOptionPane.INFORMATION_MESSAGE);
			...
			if (JOptionPane.showConfirmDialog(this, "确定删除账号吗？", "确认",
					JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				String userid = this.tblData[i][0].toString();
				try {
					(new UserManager()).deleteUser(userid);
					JOptionPane.showMessageDialog(null, "账号已删除", "提示", JOptionPane.INFORMATION_MESSAGE);
					this.reloadUserTable();
				} catch (BaseException e1) {
					JOptionPane.showMessageDialog(null, e1.getMessage(), "错误", JOptionPane.ERROR_MESSAGE);
				}
